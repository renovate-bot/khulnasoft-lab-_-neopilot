...
from neopilot.api.client.client import AuthenticatedClient
from neopilot.connection_config import ConnectionConfig
from neopilot.api.metadata import default_headers
from neopilot.exceptions import (
    AuthenticationException,
    SandboxException,
    RateLimitException,
)
from neopilot.api.client.types import Response
...
class ApiClient(AuthenticatedClient):
    """
    The client for interacting with the NEOPILOT API.
    """
        ...
        **kwargs,
        ...
        if require_api_key:
            if config.api_key is None:
                raise AuthenticationException(
                    "API key is required, please visit the Team tab at https://neopilot.dev/dashboard to get your API key. "
                    "You can either set the environment variable `NEOPILOT_API_KEY` "
                    'or you can pass it directly to the sandbox like Sandbox(api_key="neopilot_...")',
                )
        ...
        if require_access_token:
            if config.access_token is None:
                raise AuthenticationException(
                    "Access token is required, please visit the Personal tab at https://neopilot.dev/dashboard to get your access token. "
                    "You can set the environment variable `NEOPILOT_ACCESS_TOKEN` or pass the `access_token` in options.",
                )
