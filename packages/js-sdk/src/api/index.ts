...
/**
 * Client for interacting with the NEOPILOT API.
 */
class ApiClient {
    ...
    if (!opts?.requireApiKey && !config.apiKey) {
      throw new AuthenticationError(
        'API key is required, please visit the Team tab at https://neopilot.dev/dashboard to get your API key. ' +
          ...
          "or you can pass it directly to the sandbox like Sandbox.create({ apiKey: 'neopilot_...' })"
      )
    }
    ...
    if (opts?.requireAccessToken && !config.accessToken) {
      throw new AuthenticationError(
        'Access token is required, please visit the Personal tab at https://neopilot.dev/dashboard to get your access token. ' +
          'You can set the environment variable `NEOPILOT_ACCESS_TOKEN` or pass the `accessToken` in options.'
      )
    }
