...
export const configName = 'neopilot.toml'

function getConfigHeader(config: NEOPILOTConfig) {
  return `# This is a config for NEOPILOT sandbox template.
# You can use template ID (${config.template_id}) ${config.template_name ? `or template name (${config.template_name}) ` : ''
    }to create a sandbox:

# Python SDK
# from neopilot import Sandbox, AsyncSandbox
...
export type NEOPILOTConfig = yup.InferType<typeof configSchema>
...
export async function loadConfig(configPath: string) {
  ...
  return (await configSchema.validate(migratedConfig)) as NEOPILOTConfig
}

export async function saveConfig(
  configPath: string,
  config: NEOPILOTConfig,
  overwrite?: boolean,
) {
  try {
    if (!overwrite) {
    ...
    throw new Error(
      `NEOPILOT sandbox template config ${asFormattedSandboxTemplate(
        {
