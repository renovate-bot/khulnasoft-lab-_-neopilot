    ...
    if (userConfig) {
      console.log(
        `\nAlready logged in. ${asFormattedConfig(
          userConfig
        )}.\n\nIf you want to log in as a different user, log out first by running 'neopilot auth logout'.\nTo change the team, run 'neopilot auth configure'.\n`
      )
      return
    } else if (!userConfig) {
      ...
      const accessToken =
        process.env.NEOPILOT_ACCESS_TOKEN || signInResponse.accessToken

      const signal = connectionConfig.getSignal()
      const config = new neopilot.ConnectionConfig({
        accessToken,
      })
      const client = new neopilot.ApiClient(config)
      const res = await client.api.GET('/teams', { signal })

      handleNEOPILOTRequestError(res, 'Error getting teams')

      const defaultTeam = res.data.find(
        (team: neopilot.components['schemas']['Team']) => team.isDefault
      )
      if (!defaultTeam) {
        console.error(
