...
export async function buildWithProxy(
  ...
  connectionConfig: neopilot.ConnectionConfig,
  accessToken: string,
  ...
  if (!userConfig?.dockerProxySet) {
    console.log(
      'There was an issue during Docker authentication. Please follow the workaround steps from https://neopilot.dev/docs/troubleshooting/templates/build-authentication-error and then continue.'
    )
    const yes = await confirm(
      'Have you completed the steps from the https://neopilot.dev/docs/troubleshooting/templates/build-authentication-error workaround guide?'
    )

    if (!yes) {
      console.log(
        'Please follow the workaround steps from https://neopilot.dev/docs/troubleshooting/templates/build-authentication-error and then try again.'
      )
      process.exit(1)
    }
  }
  ...
  const accessTokenBase64Encoded = Buffer.from(
    `_neopilot_access_token:${accessToken}`
  ).toString('base64')
...
async function docker(
  ...
  child_process.execSync(
    `docker tag docker.${connectionConfig.domain}/neopilot/custom-envs/${template.templateID}:${template.buildID} ${localDomain}:${PORT}/neopilot/custom-envs/${template.templateID}:${template.buildID}`,
    {
      stdio: 'inherit',
      cwd: root,
    }
  ...
  const child = child_process.spawn(
    ...
    [
      'push',
      `${localDomain}:${PORT}/neopilot/custom-envs/${template.templateID}:${template.buildID}`,
    ],
    {
      detached: true,
      stdio: 'inherit',
...
async function proxy(
  ...
  const res = await fetch(
    `https://docker.${connectionConfig.domain}/v2/token?account=_neopilot_access_token&scope=repository%3Aneopilot%2Fcustom-envs%2F${template.templateID}%3Apush%2Cpull`,
    {
      method: 'GET',
      headers: {
