...
import { handleNEOPILOTRequestError } from '../../utils/errors'
...
async function getTemplateBuildLogs({
  ...
  handleNEOPILOTRequestError(res, 'Error getting template build status')
  return res.data as neopilot.paths['/templates/{templateID}/builds/{buildID}/status']['get']['responses']['200']['content']['application/json']
}

async function requestTemplateBuild(
  args: neopilot.paths['/templates']['post']['requestBody']['content']['application/json'],
) {
...
async function requestTemplateRebuild(
  templateID: string,
  args: neopilot.paths['/templates/{templateID}']['post']['requestBody']['content']['application/json'],
) {
...
async function triggerTemplateBuild(templateID: string, buildID: string) {
  ...
  handleNEOPILOTRequestError(res, 'Error triggering template build')
  return res.data
}
...
export const buildCommand = new commander.Command('build')
  ...
  .option(
    '-d, --dockerfile <file>',
    `specify path to Dockerfile. By default NEOPILOT tries to find ${asLocal(
      defaultDockerfileName,
      ...
      try {
        ...
        try {
          child_process.execSync(
            `echo "${accessToken}" | docker login docker.${connectionConfig.domain} -u _neopilot_access_token --password-stdin`,
            {
              stdio: 'inherit',
              cwd: root,
            },
          ...
          console.error(
            'Docker login failed. Please try to log in with `neopilot auth login` and try again.',
          )
          process.exit(1)
        }
        process.stdout.write('\n')
        ...
        const cmd = [
          ...
          `-t docker.${connectionConfig.domain}/neopilot/custom-envs/${templateID}:${template.buildID}`,
          buildArgs,
          noCache,
          '.',
        ].join(' ')
        ...
        const pushCmd = `docker push docker.${connectionConfig.domain}/neopilot/custom-envs/${templateID}:${template.buildID}`
        console.log(
  ...
  do {
    ...
    switch (template.status) {
      case 'building':
        break
      case 'ready': {
        const pythonExample = asPython(`from neopilot import Sandbox, AsyncSandbox
...
sandbox = await AsyncSandbox.create("${
        ...
        const typescriptExample = asTypescript(`import { Sandbox } from 'neopilot'
      ...
      case 'error':
        throw new Error(
          `\n❌ Building sandbox template ${asFormattedSandboxTemplate({
            ...
            '(https://neopilot.dev/docs/support)',
          )} to get help.\n`,
        )
    }
...
function getDockerfile(root: string, file?: string) {
  ...
  // Check if default dockerfile neopilot.Dockerfile exists
...
async function requestBuildTemplate(
  ...
  Omit<
    neopilot.paths['/templates']['post']['responses']['202']['content']['application/json'],
    'logs'
  >
> {
  let res
  ...
  handleNEOPILOTRequestError(res, 'Error requesting template build')
  return res.data
}
