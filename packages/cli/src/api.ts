import * as boxen from 'boxen'
import * as neopilot from 'neopilot'

import { getUserConfig, UserConfig } from './user'
import { asBold, asPrimary } from './utils/format'

export let apiKey = process.env.NEOPILOT_API_KEY
export let accessToken = process.env.NEOPILOT_ACCESS_TOKEN

const authErrorBox = (keyName: string) => {
  let link
  let msg
  switch (keyName) {
    case 'NEOPILOT_API_KEY':
      link = 'https://neopilot.dev/dashboard?tab=keys'
      msg = 'API key'
      break
    case 'NEOPILOT_ACCESS_TOKEN':
      link = 'https://neopilot.dev/dashboard?tab=personal'
      msg = 'access token'
      break
  }
  // throwing error in default in switch statement results in unreachable code,
  // so we need to check if link and msg are defined here instead
  if (!link || !msg) {
    throw new Error(`Unknown key name: ${keyName}`)
  }
  return boxen.default(
    `You must be logged in to use this command. Run ${asBold('neopilot auth login')}.

If you are seeing this message in CI/CD you may need to set the ${asBold(
      `${keyName}`
    )} environment variable.
Visit ${asPrimary(link)} to get the ${msg}.`,
    {
      width: 70,
      float: 'center',
      padding: 0.5,
      margin: 1,
      borderStyle: 'round',
      borderColor: 'redBright',
    }
  )
}

export function ensureAPIKey() {
  // If apiKey is not already set (either from env var or from user config), try to get it from config file
  if (!apiKey) {
    const userConfig = getUserConfig()
    apiKey = userConfig?.teamApiKey || userConfig?.defaultTeamApiKey
  }

  if (!apiKey) {
    console.error(authErrorBox('NEOPILOT_API_KEY'))
    process.exit(1)
  } else {
    return apiKey
  }
}

export function ensureUserConfig(): UserConfig {
  const userConfig = getUserConfig()
  if (!userConfig) {
    console.error('No user config found, run `neopilot auth login` to log in first.')
    process.exit(1)
  }
  return userConfig
}

export function ensureAccessToken() {
  // If accessToken is not already set (either from env var or from user config), try to get it from config file
  if (!accessToken) {
    const userConfig = getUserConfig()
    accessToken = userConfig?.accessToken
  }

  if (!accessToken) {
    console.error(authErrorBox('NEOPILOT_ACCESS_TOKEN'))
    process.exit(1)
  } else {
    return accessToken
  }
}

const userConfig = getUserConfig()

export const connectionConfig = new neopilot.ConnectionConfig({
  accessToken: process.env.NEOPILOT_ACCESS_TOKEN || userConfig?.accessToken,
  apiKey: process.env.NEOPILOT_API_KEY || userConfig?.teamApiKey,
})
export const client = new neopilot.ApiClient(connectionConfig)
