# Connect LLMs to NEOPILOT
NEOPILOT can work with any LLM and AI framework. The easiest way to connect an LLM to NEOPILOT is to use the tool use capabilities of the LLM (sometimes known as function calling).
...
<CodeGroup>
```python
# pip install openai neopilot-code-interpreter
...
# Execute code in NEOPILOT Sandbox
if code:
...
<CodeGroup>
```python
# pip install anthropic neopilot-code-interpreter
...
<CodeGroup>
```python
# pip install mistralai neopilot-code-interpreter
import os
...
# pip install groq neopilot-code-interpreter
import os
...
<CodeGroup>
```js
// npm install ai @ai-sdk/openai @neopilot/code-interpreter
...
// Run the code in NEOPILOT Sandbox
...
<CodeGroup>
```js
// npm install ai @ai-sdk/openai zod @neopilot/code-interpreter
import { openai } from '@ai-sdk/openai'
import { generateText } from 'ai'
import z from 'zod'
import { Sandbox } from '@neopilot/code-interpreter'
...
<CodeGroup>
```python
# pip install crewai neopilot-code-interpreter
...
<CodeGroup>
```python
# pip install langchain langchain-openai neopilot-code-interpreter
...
# Define a tool that uses the NEOPILOT Sandbox
neopilot_sandbox_tool = Tool(
    name="execute_python",
    func=execute_python,
    description="Execute python code in a Jupyter notebook cell and return result"
)
...
llm = ChatOpenAI(model="gpt-4o").bind_tools([neopilot_sandbox_tool])
...
<CodeGroup>
```python
# pip install llama-index neopilot-code-interpreter
...
neopilot_sandbox_tool = FunctionTool.from_defaults(
    name="execute_python",
    description="Execute python code in a Jupyter notebook cell and return result",
    fn=execute_python
)
...
agent = ReActAgent.from_tools([neopilot_sandbox_tool], llm=llm, verbose=True)
