...
When you are building a sandbox template you can specify the ready command by using the [`--ready-cmd`](/docs/sdk-reference/cli/v1.5.0/template#neopilot-template-build) option:

```bash
neopilot template build --ready-cmd "<your-ready-command>"
```

### Sandbox template config
You can specify the ready command inside the `neopilot.toml` in the same directory where you run `neopilot template build`.
