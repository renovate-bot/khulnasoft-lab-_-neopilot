...
When you are building a sandbox template you can specify the start command by using the [`-c`](/docs/sdk-reference/cli/v1.0.9/template#neopilot-template-build) option:

```bash
neopilot template build -c "<your-start-command>"
```
...
You can specify the start command also inside the `neopilot.toml` in the same directory where you run `neopilot template build`.
...
```bash {{ language: 'bash' }}
neopilot sandbox logs <sandbox-id>
```
</CodeGroup>
