...
In this guide, we'll create a custom NEOPILOT sandbox with preinstalled dependencies and files.
Once the sandbox is built, we'll show how to spawn and control it with our SDK.

## Prerequisites
1. [Node.js](https://nodejs.org/) 18.0.0 or later
1. [NEOPILOT CLI](/docs/cli/installation)
...
```bash
npm install -g @neopilot/cli@latest
```
</CodeGroup>
...
## 2. Login to CLI
Before you create your first custom sandbox, you will need to authenticate in the CLI with your NEOPILOT account.
...
<Note>
You need to have an existing NEOPILOT account to login. Sign up [here](/docs/getting-started/api-key).
</Note>

## 3. Create `neopilot.Dockerfile`
To describe how your custom sandbox will look like, create a new Dockerfile and name it `neopilot.Dockerfile`.
We use this Dockerfile as the [template file](/docs/sandbox/templates/template-file).

<Note>
Run `neopilot template init` to create `neopilot.Dockerfile` in the current directory.
</Note>
...
<CodeGroup isFileName title="neopilot.Dockerfile" isRunnable={false}>
```bash
...
Now it's time to create your custom sandbox based on the sandbox template file (the `neopilot.Dockefile` file) you just created in the previous step.
...
```bash
neopilot template build --name "my-agent-sandbox"
```
</CodeGroup>
...
Found ./neopilot.Dockerfile that will be used to build the sandbox template.
...
┌ Usage examples ──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
│                                                                                                                                                      │
│  You can use NEOPILOT Python or JS SDK to spawn sandboxes now.                                                                                            │
│  Find more here - https://neopilot.dev/docs/guide/custom-sandbox in Spawn and control your sandbox section.                                               │
│                                                                                                                                                      │
│───────────────────────────────────────────────────────────────────── Python SDK ─────────────────────────────────────────────────────────────────────│
│                                                                                                                                                      │
...
│  # https://neopilot.dev/docs/sandbox/overview                                                                                                             │
│                                                                                                                                                      │
...
This will create the `neopilot.toml` file storing the sandbox config.
<CodeGroup isFileName title="neopilot.toml" isRunnable={false}>
```toml
# This is a config for NEOPILOT sandbox template
template_id = "1wdqsf9le9gk21ztb4mo"
dockerfile = "neopilot.Dockerfile"
...
Now you can use the [NEOPILOT SDK](/docs/getting-started/installation) to spawn & control your new custom sandbox.
