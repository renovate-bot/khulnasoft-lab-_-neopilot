...
The following guide shows how to migrate your code from the current Core SDKs (`neopilot`) and Code Interpreter SDKs (`@neopilot/code-interpreter`/`neopilot-code-interpreter`) to the beta versions.

The latest beta versions you should install can be found in NPM/PyPi release history:
- **JavaScript & TypeScript**
	- [Latest release](https://www.npmjs.com/package/neopilot?activeTab=versions) with *rc* tag for `neopilot`
	- [Latest release](https://www.npmjs.com/package/@neopilot/code-interpreter?activeTab=versions) with *rc* tag for `@neopilot/code-interpreter`
- **Python**
	- [Latest release](https://pypi.org/project/neopilot/#history) with *PRE-RELEASE* for `neopilot`
	- [Latest release](https://pypi.org/project/neopilot-code-interpreter/#history) with *PRE-RELEASE* for `neopilot-code-interpreter`
...
These changes affect both the `neopilot` Core SDKs and Code Interpreter SDKs (`@neopilot/code-interpreter`/`neopilot-code-interpreter`) because the Code Interpreter uses the Core SDK.

<Note>
If you are using a custom template you need to rebuild it by calling `neopilot template build` in the directory with the template.
</Note>
...
If you need this feature please reach out to us on [Discord](https://discord.com/invite/U7KEcGErtQ) or [GitHub](https://github.com/khulnasoft-lab/neopilot) so we can prioritize it.
...
<CodeGroup isRunnable={false}>
```js
import CodeInterpreter from '@neopilot/code-interpreter'
...
```python
from neopilot_code_interpreter import CodeInterpreter
...
```
if you use the NEOPILOT Docker image as base in your `neopilot.Dockerfile`
```docker
...
Or if you are using custom base you need to copy all files excluding the `neopilot.toml` from [beta template](https://github.com/khulnasoft-lab/code-interpreter/tree/beta/template)
to the directory with your template and then run `neopilot template build -c "/root/.jupyter/start-up.sh"`.
...
from neopilot_code_interpreter import AsyncCodeInterpreter
