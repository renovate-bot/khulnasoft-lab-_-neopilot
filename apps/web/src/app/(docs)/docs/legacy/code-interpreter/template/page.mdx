...
If you want to customize the Code Interprerter sandbox (e.g.: add a preinstalled package) you can do that by using a [custom sandbox template](https://neopilot.dev/docs/sandbox/templates/overview).


## Step-by-step guide
1. Create custom sandbox by following [this guide](https://neopilot.dev/docs/guide/custom-sandbox)

2. Use prebuilt [NEOPILOT Code Interpreter image](https://hub.docker.com/r/neopilotdev/code-interpreter) by replacing the `FROM` command in your `neopilot.Dockerfile` with following

    ```sh
    FROM neopilotdev/code-interpreter:latest
    ```

3. Run the following in the same directory where's your `neopilot.toml`
    ```sh
...
```sh
neopilot template build -c "/home/user/.jupyter/start-up.sh" --cpu-count 4 --memory-mb 4096
```
